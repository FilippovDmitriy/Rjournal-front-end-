...
const AuthPopup: FC<Props> = ({isOpen, setIsOpen, userData}) => {
    const [authType, setAuthType] = useState<'login' | 'register'>('register');
    const [formType, setFormType] = useState<'main' | 'mail'>('main');
    ...
    return (
        <div className={styles.headerAuth}>
            <Button className={styles.authButton} variant={'text'} onClick={handleClickOpen}>
                <PersonIcon/>
                Войти
            </Button>
            <Dialog
            ...
            >
                {formType !== 'main' &&
                <span className={styles.back} onClick={() => setFormType('main')}><ArrowIcon/>Назад</span>}
                <DialogTitle>
                    <Typography variant={'h4'}>
                        {authType === 'register' && 'Регистрация'}
                        {authType === 'login' && 'Вход в аккаунт'}
                    </Typography>
                </DialogTitle>
                <DialogContent>
                    <div className={styles.content}>
                        {formType === 'main' && <>
                            <div className={styles.fullButtons}>
                                <Button onClick={() => setFormType('mail')} variant={'contained'} fullWidth>
                        ...
                        {formType === 'mail' && <MailForm authType={authType}/>}
                        {authType === 'register' && <div className={styles.already}>
                            <p>Есть аккаунт?</p>
                            <span onClick={() => setAuthType('login')}
                                  className={classNames('textLink', styles.link)}>Войти</span>
                        </div>}
                        <div className={styles.links}>
                            {authType === 'login' && <span onClick={() => setAuthType('register')}
                                                           className={classNames('textLink', styles.link)}>Регистрация</span>}
                        </div>
                    </div>
                    <Link href={'/rating'}>
                        <a className={classNames('textLink', styles.link, styles.term)}>
                            {authType === 'register' && 'Условия использования'}
                        </a>
                    </Link>
                </DialogContent>
            </Dialog>
